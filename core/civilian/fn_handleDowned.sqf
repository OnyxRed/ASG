...
*/
private["_unit","_shooter","_curWep","_curMags","_attach","_rubber"];
_unit = [_this,0,Objnull,[Objnull]] call BIS_fnc_param;
_shooter = [_this,1,Objnull,[Objnull]] call BIS_fnc_param;
_rubber = [_this,2,true] call BIS_fnc_param;
...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_isdowned) then
	{
        ...
        if (_rubber) then{
            [[0,format["%1 was incapacitated with rubber bullets by %2", name _unit, name _shooter]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
        } else {
            [[0,format["%1 was incapacitated by %2", name _unit, name _shooter]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
        };
