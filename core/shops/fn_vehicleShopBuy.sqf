...
*/
private["_mode","_spawnPoints","_className","_basePrice","_colorIndex","_spawnPoint","_vehicle","_useBankAcc","_allowBankAcc","_markerPos"];
_mode = _this select 0;
if((lbCurSel 2302) == -1) exitWith {hint localize "STR_Shop_Veh_DidntPick"};
_className = lbData[2302,(lbCurSel 2302)];
_vIndex = lbValue[2302,(lbCurSel 2302)];
_vehicleList = [life_veh_shop select 0] call life_fnc_vehicleListCfg; _basePrice = (_vehicleList select _vIndex) select 1; _rentalOnly = (_vehicleList select _vIndex) select 2; _allowBankAcc = (_vehicleList select _vIndex) select 3;
if (isNil "_allowBankAcc") then { _allowBankAcc = false };
if (isNil "_rentalOnly") then { _rentalOnly = false };
_useBankAcc = false;
...
if ((_allowBankAcc) && (life_cash < _basePrice )) then { _useBankAcc = true };
if ((_useBankAcc) && (life_atmcash < _basePrice)) exitWith {hint format[localize "STR_Shop_Veh_NotEnoughBank",[_basePrice - life_atmcash] call life_fnc_numberText];};
if (!(_useBankAcc) && (life_cash < _basePrice)) exitWith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - life_cash] call life_fnc_numberText];};
...
if((life_veh_shop select 0) == "med_air_hs") then {
	...
	_vehicle setPos [_markerPos select 0,_markerPos select 1,(_markerPos select 2) + 0.5];
	[[_vehicle,_colorIndex],"life_fnc_colorVehicle",true,false] spawn life_fnc_MP;
	[_vehicle] call life_fnc_clearVehicleAmmo;
    _vehicle setCollisionLight true;
	[[_vehicle,"trunk_in_use",false,true],"TON_fnc_setObjVar",false,false] spawn life_fnc_MP;
...
switch(playerSide) do {
	case west: {
        ...
        _vehicle spawn life_fnc_initTFARRadioLR_veh;
        if (__GETC__(life_coplevel) == 13) then {
            _vehicle disableTIEquipment false;
			_vehicle setVariable["trunk",[[["gpstracker",5],["handCuffKey",1],["fuelF",1],["defusekit",1],["civdefusekit",1]],27]];
			_vehicle setVariable["tracking",true,true];
			[_vehicle] spawn {
				_veh = _this select 0;
	...
	case civilian:
	{
        ...
        if (license_civ_bh) then{_vehicle setVariable["bhCar",true,true];};
	};
