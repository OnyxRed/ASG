...
_msg = format["This person has just been caught by a fixed safety camera and received a fixed penalty notice of £%1 for doing %2 km/h in a %3 zone.",[_ticket] call life_fnc_numberText,round _speed,_limit];
_msg2 = format["This person has just been caught by a fixed safety camera and received a fixed penalty notice of £%1 for doing %2 km/h in a %3 zone and the camera reporting a failure to pay.",[_ticket] call life_fnc_numberText,round _speed,_limit];
_msg3 = format["This person has just been caught by a fixed safety camera and received a fixed penalty notice of £%1 for doing %2 km/h in a %3 zone and the registration scan says this driver has no license.",[_ticketWL] call life_fnc_numberText,round _speed,_limit];
_msg4 = format["This person has just been caught by a fixed safety camera and received a fixed penalty notice of £%1 for doing %2 km/h in a %3 zone and the registration scan says this driver has no license.",[_ticketWL] call life_fnc_numberText,round _speed,_limit];
...
if (_speed > _limit) then {
	if (side _driver == civilian) then {
		...
		if(license_civ_driver OR license_civ_truck ) then {
			...
			if(life_speedTicket <= 3) then {systemChat format["This is fixed penalty notice number %1 for you!",life_speedTicket];};
			if(life_speedTicket == 4) then {systemChat format["This is fixed penalty notice number %1 for you, next one will take away your drivers license!",life_speedTicket];};
			if(life_speedTicket > 4) exitWith
			{
					[2,-1,_msg3] call life_fnc_newMsg;
					systemChat format["This is fixed penalty notice number %1 for you!",life_speedTicket];
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Driver: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Your drivers licence has been revoked and you are wanted for repeated speed offences.",round _speed,_limit,name _driver];
			...
			if(life_cash <= _ticket) then
			{
				...
				[0] call SOCK_fnc_updatePartial;
					...
					if(life_speedCaught > 6) exitWith {
							...
							hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Driver: %3<br/>You are now wanted for Speeding.",round _speed,_limit,name _driver];
							[[getPlayerUID _driver,name _driver,"120WL"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
							
					};
					[2,-1,_msg3] call life_fnc_newMsg;
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Driver: %3<br/><t color='#ffffff'><t align='center'><t size='1'>You are now wanted for driving without a licence and may receive a fixed penalty notice.",round _speed,_limit,name _driver];
					[[getPlayerUID _driver,name _driver,"120WL"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
