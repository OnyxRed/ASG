...
if(visibleMap) then {
	...
	{if(_x getVariable ["stolen",false] && _x getVariable["tracking",false]) then { _victims pushBack _x; };} forEach vehicles;
	//Create markers
	{
	...
	
	{
		...
		_vMarker setMarkerTextLocal format["Stolen %1", getText(configFile >> "CfgVehicles" >> (typeOf _x) >> "displayName")];
