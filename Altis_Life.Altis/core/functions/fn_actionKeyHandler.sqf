...
if(_curTarget isKindOf "Man" && {!alive _curTarget} && {playerSide in [west,independent]}) exitWith {
	//Hotfix code by ins0
	if(((playerSide == blufor && {(call life_revive_cops)}) || (license_baf_cmedic) || playerSide == independent) && {"Medikit" in (items player)}) then {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	_miscItems = ["Land_BottlePlastic_V2_F","Land_TacticalBacon_F","Land_Can_V3_F","Land_CanisterFuel_F","Land_Suitcase_F"];
