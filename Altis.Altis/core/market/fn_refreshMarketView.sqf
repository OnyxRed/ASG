...
else
{
	...
	
	{
		if((_x select 0) == _shortname) exitWith
		{
			...
			{
				ctrlSetText [1200, "icons\market\trendup.paa"];
			}
			else
			{
				ctrlSetText [1200, "icons\market\trenddown.paa"];
			};
			...
			{
				ctrlSetText [1201, "icons\market\trendup.paa"];
			}
			else
			{
				ctrlSetText [1201, "icons\market\trenddown.paa"];
			};
		};
	}
