...
if(((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) && ({(side _x == west) && (_x getVariable["rank",0] != 12)} count playableUnits < 10)) exitWith {hint "There must be at least 10 cops online to continue."};
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
  ...
  if (_building getVariable["houseAlarm",false]) then {
    [_building,player] spawn life_fnc_houseAlarmAlert;
  };
};
