[] spawn  {
	...
	_fnc_food = 
	{
		if(life_hunger < 2) then {player setDamage 1.028131; hint localize "STR_NOTF_EatMSG_Death";}
		else
		{
	...
	_fnc_water = 
	{
		if(life_thirst < 2) then {player setDamage 1.028131; hint localize "STR_NOTF_DrinkMSG_Death";}
		else
		{
...
[] spawn
{
	while {true} do
	{
		private["_damage"];
		sleep 1;
		while {((player distance (getMarkerPos "Warm_marker") < 3101) && (player getVariable["Revive",TRUE]))} do
		{
			if(uniform player == "U_C_Scientist") then
			{
				hint "!!! You are in a radioactive area !!! You have protective clothing on!";
				sleep 5;
			} else {
				hint "!!! ATTENTION YOU ARE IN A RADIOACTIVE ZONE !!! You could die if you're not protected";
				_damage = damage player;
				_damage = _damage + 0.1;
                if (_damage >= 1) then{_damage = 1.028131};
...
[] spawn  {
	while{true} do
	{
		...
		while{(player getVariable "missingOrgan")} do {
			life_thirst =  50;
			life_hunger =  50;
			if(damage player < (1 - life_max_health)) then {_damage = (1 - life_max_health);};
            if (_damage >= 1) then{_damage = 1.028131};
            player setDamage = _damage;
